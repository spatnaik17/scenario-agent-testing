...
Pro tip: This is also the best approach when developing new features or responding to bug reports. Don't jump into code or prompt changes: **write a scenario first**. You can even use [Debug Mode](/basics/debug-mode) to vibe-debug directly in the terminal.
...
Check out the [Agent Testing Pyramid Guide](/best-practices/the-agent-testing-pyramid), that shows how to combine different testing strategies, so you can really cover your agent quality from all angles.
