...
1. **Agent Integration**: You created an <RefLink link="agent_adapter.html#scenario.agent_adapter.AgentAdapter">`AgentAdapter`</RefLink> that wraps your agent function
2. **Simulation**: The <RefLink link="user_simulator_agent.html">`UserSimulatorAgent`</RefLink> automatically generated realistic user messages based on the scenario description
3. **Evaluation**: The <RefLink link="judge_agent.html">`JudgeAgent`</RefLink> evaluated the conversation against your criteria
4. **Caching**: The <RefLink link="index.html#scenario.cache">`@scenario.cache()`</RefLink> decorator made your agent calls deterministic for testing
...
- Learn about [Scenario Basics](/scenario/concepts) to understand the framework deeply
